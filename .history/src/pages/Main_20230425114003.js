...
import dataControls from "../data/dataControls.json";
import dataDisplay from "../data/dataForTable.json";
import dataMappingForDisplay from "../data/dataMapping.json";
...
const dataControlObject = JSON.parse(JSON.stringify(dataControls))

const MainComponent = () => {
    const [objectStates, setObjectState] = useState(dataControlObject)
    ...
    const [mappingDisplay, setMappingDisplay] = useState(dataMappingDisplay)
    const [finalDisplays, setFinalDisplays] = useState([])
    ...
    //     const controlName = e.currentTarget.name
    //     const controlVal = e.currentTarget.value
        
    //     setObjectState(objectStates.map(objectState => {
    //         if (objectState.name === controlName) {
    //             if(objectState.name === 'mount')
    //                 showInsideOutsideFrame(controlVal, setShowInsideFrame, setShowOutsideFrame)
    //             if(objectState.name === 'dividerSplitOption')
    //                 showHideDividerSplit(controlVal, setShowDividerSplit)    
    //             return {...objectState, value: controlVal}
    //         } else {
    //             return objectState
    //           }
    //       })) 
    ...
    return (  
        <div className="app">
            ...
            <div className="form-main-position">
                ...
                <button type="button" onClick={calculate}>Save</button>
            </div>    
            </form>
            { showHideTable && <TableDetailMain options={dataForTables} setdatafortable={setDataForTables} showHideTable={setShowHideTable} ></TableDetailMain> }
            <br></br>
            <br></br>
        </div>
    );

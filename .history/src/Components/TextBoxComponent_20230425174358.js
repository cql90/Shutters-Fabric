...
function setStyle(error, fieldName, type) {
  if (getIn(errors, fieldName)) {
    if(type === 0)
      return { border: '1px solid red' }
    return { color: 'red' }
...
const TextBoxComponent = ({ label, ...props }) => {
  ...
  return (
    ...
    <div className={props.classdiv}>
      ...
      <input style={meta.error && meta.touched? setStyle(function setStyle(errors, fieldName, type) {
  if (getIn(errors, fieldName)) {
    if(type === 0)
      return { border: '1px solid red' }
    return { color: 'red' }
  }
}, props.name, 0): null}className="text-input" {...field} {...props} />
